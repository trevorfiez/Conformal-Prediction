...
tf.app.flags.DEFINE_string('data_dir', '/scratch/tfiez/conformal/cifar10/cifar10_data',
...
def inference(images):
  ...
  with tf.variable_scope('sigmoid_linear') as scope:
    ...
    sigmoid_linear = tf.sigmoid(tf.matmul(local4, weights) + biases, name=scope.name)
    _activation_summary(sigmoid_linear)

  return sigmoid_linear

def pre_inference(images):
  """Build the CIFAR-10 model.
  ...
  with tf.variable_scope('sigmoid_linear') as scope:
    ...
    biases = _variable_on_cpu('biases', [NUM_CLASSES],
	                      tf.constant_initializer(0.0))
    pre_sigmoid_linear = tf.matmul(local4, weights) + biases
    #_activation_summary(pre_sigmoid_linear)

  return pre_sigmoid_linear
...
def loss(predictions, labels):
  ...
  tf.add_to_collection('losses', logloss_mean)
